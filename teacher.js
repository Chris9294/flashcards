...
async function loadData() {
  // Charger thèmes depuis Supabase
  ...
  // Charger cartes depuis Supabase
  const { data: cardsData, error: cardsError } = await supabase.from('cards').select('*');
  if (checkError(cardsError, "Erreur chargement des cartes")) return;
...
async function addTheme() {
  ...
  const themeObj = { id: newTheme.id.toString(), name: newTheme.name, cards: [] };
...
function refreshCards() {
  ...
  theme.cards.forEach((card, index) => {
    ...
    const audioInfo=document.createElement('p'); const audioText=document.createElement('span');
    ...
    playBtn.onclick=()=>{ if(card.audio) new Audio(card.audio).play(); else { const utter=new SpeechSynthesisUtterance(card.word); utter.lang='en-GB'; utter.rate=0.7; speechSynthesis.cancel(); speechSynthesis.speak(utter); } };
    ...
    div.appendChild(deleteBtn); div.appendChild(toggleBtn);
...
async function importZip(file, themeId) {
  ...
  const images={}; const audios={}; const tasks=[];

  zip.forEach((path,entry)=>{
    if(entry.dir) return; if(path.startsWith('__MACOSX')) return;
    const filename=path.split('/').pop(); const ext=filename.split('.').pop().toLowerCase();
  ...
  for(const name of Object.keys(images)){
    ...
    if(!error){ theme.cards.push({id:newCard.id,word:newCard.word,image:imageUrl,audio:audioUrl,visible:true}); added++; }
  }
