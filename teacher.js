...
const supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNkcndqZ3lsbWJncmhmd25waHdhIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI3MjU0NDksImV4cCI6MjA4ODMwMTQ0OX0.PR1RnWJbVKbQx4Hm-TQEjLrW36INk5Wv-BX0sCXg_To";
...
async function loadData() {
  ...
  cardsData.forEach(card => {
    ...
    if (!themesMap[themeId]) themesMap[themeId] = { id: themeId, name: `Série ${themeId}`, cards: [] };
...
async function uploadFileBase64(data64, filename) {
  ...
  const { data, error } = await supabase.storage.from('cards').upload(uniqueName, blob, { upsert: true });
  if (error) { console.error("Erreur upload base64:", error); return null; }
...
function addTheme() {
  ...
  const newTheme = { id: themeId.toString(), name, cards: [] };
...
function refreshThemes() {
  ...
  placeholder.value = ''; placeholder.textContent = '— Choix de la série —'; placeholder.disabled = true;
...
deleteThemeBtn.onclick = () => {
  ...
  if (confirm(`Supprimer la série "${theme.name}" ?`)) {
    theme.cards.forEach(card => { if (card.id) supabase.from('cards').delete().eq('id', card.id); });
    data.themes = data.themes.filter(t => t.id !== theme.id);
    refreshThemes(); refreshCards();
  }
};
...
async function addCard() {
  ...
  document.getElementById('wordInput').value = ''; imageInput.value = ''; audioInput.value = '';
  updateFileLabel(imageInput); updateFileLabel(audioInput);
  refreshCards();
}
...
async function addAudioToCard(index, file) {
  ...
  card.audio = audioUrl; refreshCards();
}
...
function refreshCards() {
  ...
  const div = document.createElement('div'); div.className = 'card'; if (!card.visible) div.classList.add('card-hidden');
    ...
    saveBtn.onclick = async () => { card.word = wordInput.value.trim(); await supabase.from('cards').update({ word: card.word }).eq('id', card.id); refreshCards(); };

    const img = document.createElement('img'); img.src=card.image; img.style.height='60px';
    const audioInfo = document.createElement('p'); const audioText = document.createElement('span'); audioText.textContent=card.audio?'Audio : oui ':'Audio : non (synthèse vocale GB par défaut) ';
    const playBtn = document.createElement('button'); playBtn.textContent='🔊'; playBtn.onclick = () => {
    ...
    const upBtn = document.createElement('button'); upBtn.textContent='🔼'; upBtn.disabled=index===0;
    upBtn.onclick = async () => { if(index===0) return; [theme.cards[index-1],theme.cards[index]]=[theme.cards[index],theme.cards[index-1]]; refreshCards(); };

    const downBtn = document.createElement('button'); downBtn.textContent='🔽'; downBtn.disabled=index===theme.cards.length-1;
    downBtn.onclick = async () => { if(index===theme.cards.length-1) return; [theme.cards[index+1],theme.cards[index]]=[theme.cards[index],theme.cards[index+1]]; refreshCards(); };

    const deleteBtn = document.createElement('button'); deleteBtn.textContent='🗑️';
    deleteBtn.onclick = async () => { await supabase.from('cards').delete().eq('id', card.id); theme.cards.splice(index,1); refreshCards(); };

    const toggleBtn = document.createElement('button'); toggleBtn.textContent=card.visible?'👁️ Visible':'🚫 Masquée';
    toggleBtn.onclick = async () => { card.visible=!card.visible; await supabase.from('cards').update({visible:card.visible}).eq('id',card.id); refreshCards(); };

    div.appendChild(number); const editRow=document.createElement('div'); editRow.style.display='flex'; editRow.style.alignItems='center'; editRow.style.gap='6px';
    editRow.appendChild(wordInput); editRow.appendChild(saveBtn); div.appendChild(editRow);

    div.appendChild(upBtn); div.appendChild(downBtn); div.appendChild(deleteBtn); div.appendChild(toggleBtn);

    const mediaRow=document.createElement('div'); mediaRow.style.display='flex'; mediaRow.style.alignItems='center'; mediaRow.style.gap='10px';
    mediaRow.appendChild(img); mediaRow.appendChild(audioInfo); div.appendChild(mediaRow);
...
async function importZipFromInput() {
  ...
  importZip(zipInput.files[0], themeSelect.value); zipInput.value='';
}
...
async function importZip(file, themeId) {
  ...
  const images={}, audios={}, tasks=[];
  zip.forEach((path,entry)=>{
    if(entry.dir) return; if(path.startsWith('__MACOSX')) return;
  ...
  for(const name of Object.keys(images)){
    ...
    const {data:newCard,error}=await supabase.from('cards').insert([{theme_id:parseInt(themeId), word:name, image_url:imageUrl, audio_url:audioUrl, visible:true}]).select().single();
